...
// ─── Public: image overrides for static products ─────────────────────────────
...
// ─── Admin: image overrides for static products ───────────────────────────────
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (isNaN(id)) { res.status(400).json({ error: "invalid id" }); return; }
    await db
      .delete(favoriteCustomProductsTable)
      .where(eq(favoriteCustomProductsTable.id, id));
    res.json({ success: true });
  }
);
...
router.post(
  "/admin/favorites/custom-products/:id/image",
  requireAdminAuth,
  async (req: Request, res: Response): Promise<void> => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) { res.status(400).json({ error: "invalid id" }); return; }
